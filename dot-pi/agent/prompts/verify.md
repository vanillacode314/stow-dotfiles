...
- Do NOT run commands that modify state (no jj squash, no jj git push, no writes, no installs)
- Bash commands may ONLY read or inspect (ls, find, rg, jj log, jj diff, etc.)
...
- Check changeset (`jj diff`)
- If neither has changes, review current changeset vs main (`jj diff --from main --to @`)
