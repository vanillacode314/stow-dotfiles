...
M.prompt = {
	...
	prompts = {
		{
			role = "user",
			content = [[1. Explain #{mcp:neovim://diagnostics/buffer} for #{buffer} issues concisely in an easy to understand way.
...
3.1 If the user approves then apply the fix using the @{neovim__edit_file} tool and then format using the mcp @{neovim__format} tool.
